...
(function () {
  ...
  function verdict(v) {
    if (v > 150) return "不信。两个人之间到不了这个数 —— 要么你哪项重复算了,要么你在哄自己。回去重来。";
    if (v > 100) return "爆表了。这人在你这儿,早就不是一把尺子量得住的了。";
    if (v <= 0) return "陌生人。或者,你根本不想为这个人动手。";
    if (v < 15) return "点头之交。下次见面可能还得想想名字。";
    if (v < 35) return "认识,也有点意思,但还没走进心里。";
    if (v < 55) return "朋友了。会主动找,会突然想起。";
    if (v < 80) return "挚友这一档。我那个「挚友 a」就落在这附近(66.6)。";
    return "很重的人。真要割舍,会疼 —— 不过还没到爆表。";
  }
